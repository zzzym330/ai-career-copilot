    ...
    <div className="space-y-5">
      <ContentSection>
        <div className="flex flex-col justify-between gap-3 border-b border-neutral-200 pb-4 sm:flex-row sm:items-start">
          ...
          <button
            type="button"
            onClick={onReset}
            className="h-10 shrink-0 rounded-full border border-neutral-200 px-4 text-sm font-medium text-neutral-600 transition hover:border-neutral-400 hover:text-neutral-950"
          >
            重新上传
          </button>
        </div>
      ...
      <div className="flex justify-end">
        <button
          type="button"
          onClick={onGenerate}
          disabled={profile.targetDirections.length === 0}
...
function EntrySection({
  ...
  return (
    <section className="border-b border-neutral-200 py-4">
      ...
      <div className="mt-4 divide-y divide-neutral-200">{children}</div>
      <button
        type="button"
        onClick={onAdd}
        className="mt-4 text-sm font-medium text-neutral-500 transition hover:text-neutral-950"
      >
        + 新增
      </button>
    </section>
...
function DisplayItem({
  ...
  return (
    <div className="flex flex-col justify-between gap-3 py-4 first:pt-0 sm:flex-row">
      <div>
        <p className="font-semibold text-neutral-950">{title || "未命名条目"}</p>
        <p className="mt-1 text-base font-medium text-neutral-500">{subtitle}</p>
        <p className="mt-2 text-base leading-7 text-neutral-500">{description}</p>
      </div>
...
function EditForm({
  ...
  return (
    <div className="grid gap-3 py-4 first:pt-0 sm:grid-cols-2">
      {fields.map((field) => (
        <label
          key={field.key}
        ...
        >
          ...
          {field.multiline ? (
            <textarea
              ...
              className="mt-2 min-h-20 w-full resize-y rounded-xl border border-neutral-200 bg-neutral-50 px-3 py-2 text-base leading-7 outline-none focus:border-neutral-400 focus:bg-white focus:ring-4 focus:ring-neutral-950/5"
            />
          ) : (
            <input
              value={field.value}
              onChange={(event) => onChange(field.key, event.target.value)}
              className="mt-2 h-10 w-full rounded-xl border border-neutral-200 bg-neutral-50 px-3 text-base outline-none focus:border-neutral-400 focus:bg-white focus:ring-4 focus:ring-neutral-950/5"
            />
          )}
        </label>
      ))}
      <div className="flex gap-3 sm:col-span-2">
        ...
        <button
          type="button"
          onClick={onCancel}
          className="rounded-full border border-neutral-200 px-4 py-2 text-sm font-medium text-neutral-500"
        >
          取消
        </button>
      </div>
...
function SimpleListEditor({
  ...
  return (
    <section className={`py-4 ${isLast ? "" : "border-b border-neutral-200"}`}>
      <h3 className="text-lg font-semibold text-neutral-950">{title}</h3>
...
function SimpleListItem({
  ...
  return (
    <div className="flex min-h-11 items-center justify-between gap-4 rounded-xl border border-neutral-200 bg-neutral-50 px-3">
      {isEditing ? (
        <input
          autoFocus
          value={draft}
        ...
        <span className="text-base font-medium text-neutral-700">{item}</span>
      )}
      <div className="flex shrink-0 gap-3 text-sm font-medium">
        <button
          type="button"
